...
function user(host, port, usession) {
    ...
    socket.on('connect', function() {
        ...
        var userid = Math.floor(Math.random()*(config.user_id.to-config.user_id.from+1)+config.user_id.from);

        // emit data to nodejs application
        socket.emit('data', {'user_id' : userid});
        ...
        console.log('spawned user_id: '+userid+' - kill in '+(parseInt(disconnectAfter))+' seconds');
    });
}

for(var i=0; i<rampup_users; i++) {

    setTimeout(function() { user(config.host, config.port,i); }, i * rampupTimeout * 1000);
}
...
var step_two = function() {
    ...
    setInterval(function(){
        ...
        user(config.host, config.port,0);
